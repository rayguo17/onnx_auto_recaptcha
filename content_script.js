...
function getBase64(url) {
  return new Promise((resolve,reject)=>{
    //两件事：1.生成base64 并resolve
     //2. 产生一个新的thread，去进行图像数据处理，并返回结果。
    let img = new Image();
    img.src = url;
    var height,width;
    ...
    img.onload = ()=>{
      width = img.width;
      height = img.height;
      let pre_width = width*2+20;
      let pre_height = height;
      canvas.width = pre_width;
      canvas.height = pre_height;
      ctx.drawImage(img,10,0);
      data_process(ctx.getImageData(0,0,pre_width,pre_height),pre_width,pre_height);
      ctx.clearRect(0,0,pre_width,pre_height);
      canvas.width = width;
...
function data_process(imageData,width,height){
  return new Promise(async (resolve,reject)=>{
    let newWidth = (width/height)*64;
    let newHeight = 64;
    let resizedBuffer = await pica().resizeBuffer({
      src:imageData.data,
      alpha:true,
      width:width,
      height:height,
    ...
    for(let i=0;i<resizedBuffer.length;i+=4){
      ...
      let g = resizedBuffer[i+1];
      let b = resizedBuffer[i+2];
    ...
    for(let i=0;i<inputArray.length;i++){
      inputArray[i]=((inputArray[i]/255)-0.5)/0.5;
    }
    console.log(inputArray);
    let inputs = new ort.Tensor("float32",inputArray,[1,1,64,newWidth]);
    try {
      ...
      let last_item=0;
      let last_two_item=0;
      let breif = output.output.data;
      for(let i=0;i<output.output.data.length;i++){
          if(breif[i]==last_item|| breif[i]==last_two_item){
              
          }else{
              res.push(charset[breif[i]]);
          }
          last_two_item = last_item;
          last_item = breif[i]
      }
      //console.log(res);
      let capt = res.join('');
      ...
      let result = write(capt);
      if(result.error){
        console.log("pls fill in your jaccount");
        return;
      }
...
function write(value){
  ...
  let account_in = document.getElementById("user");
  if(jacc === undefined || pass===undefined){
    return {error:"error"};
  }
  account_in.value = jacc;
  let pass_in = document.getElementById("pass");
  pass_in.value = pass;
...
const charset = ["", "笤", "谴", "膀", "荔", "佰", "电", "臁", "矍", "同", "奇", "芄", "吠", "6",
